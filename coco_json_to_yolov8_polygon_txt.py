...
def convert_to_yolo(input_images_path, input_json_path, output_images_path, output_labels_path):
    ...
    f = open(file=input_json_path, mode='r')
    data = json.load(f)
    f.close()
    ...
    file_names = []
    for filename in os.listdir(path=input_images_path):
    ...
    for filename in file_names:
        ...
        if img_ann:
            with open(file=os.path.join(output_labels_path, f"{os.path.splitext(filename)[0]}.txt"), mode="a") as file_object:
                for ann in img_ann:
                    current_category = ann['category_id'] - 1
                    polygon = ann['segmentation'][0]
                    normalized_polygon = [format(
                        coord / img_w if i % 2 == 0 else coord / img_h, '.6f') for i, coord in enumerate(polygon)]
                    file_object.write(
